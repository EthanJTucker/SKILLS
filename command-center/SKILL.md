...
to adversarial review, loop until it is truly clean, and hand every
...
1. **You orchestrate. You do not write feature code, and you never run
   ...
   hygiene-or-PII greps in your own shell. Spawn a subagent to do it.** Use a
   quick "verifier" agent for the trust-but-verify pre-filter (re-run the suite,
   recompute the headline numbers from scratch, grep the diff, report a COMPACT
   pass/fail + the numbers), and use the review panel for the gate. Running
...
parallel and blind to each other (a single reviewer is rarely enough outside of one-line changes, and one
soft pass misses real flaws). The **headline / most-consequential PRs** of a project
(the ones every downstream number or consumer flows through) gets a **four-lens
"ultra" panel** (Section 5) made up of **four** independent reviewers plus adversarial
...
Re-review at the same count as the base (two reviewers for a routine PR, or the
...
- **Reviewers and fix-verifiers** work on a *detached* worktree at the PR head:
  `git worktree add --detach <path> origin/<branch>`. Treat it as read-only for
  git, and copy it to a temp dir for any experiment that mutates files.
...
- **Prune spent worktrees** once their branch is merged. Before removing,
  ...
  tracked files. Removal is destructive, so do it when idle, and it is fine to
  ask first.
...
Launch pattern: adapt it to your OS/shell. The requirements are the same either
...
1. **Role disambiguation up top.** "You spawn in the orchestrator's scratch
   worktree. That's expected: ignore it and `cd` to YOUR worktree immediately.
   No PR exists yet, you create it. You are the CODING agent, not the reviewer."
...
5. **Codebase contracts**: the API signatures, data invariants, and "read the
   ...
   panels caught real bugs here, so encode what they found.
...
- **Four independent lens reviewers**, blind to each other and to prior PR
  ...
  3. **Test-suite mutation**: introduce single-point bugs one at a time. Any
     ...
     against silent wrong numbers, so measure whether it has teeth.
...
- **Length / surface-feature confounds.** A detector may be reading a trivial
  ...
  confound, and state directly if the signal is mostly the confound.
- **Leakage across the train/test boundary**: duplicate or paraphrase rows
  straddling the split, scaler/pooling fit on all rows, or folds losing their
